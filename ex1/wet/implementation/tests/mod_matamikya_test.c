...
static double buy2Get1ForHalfOff(MtmProductData basePrice, const double amount){
    double realAmount = amount;
    if( realAmount % 3 ==0 ){
        realAmount = realAmount * (2/3);
    }
    if( realAmount % 3 == 1 ){
        realAmount = (realAmount-1)*(2/3) + 1;
    }
    if( realAmount % 3 == 2 ){
        realAmount = (realAmount-2)*(2/3) + 2;
    }
    return simplePrice(basePrice, realAmount);
}
...
bool testModifyOrders() {
    ...
    ASSERT_OR_DESTROY(order3 > 0);
    ASSERT_OR_DESTROY(MATAMIKYA_INSUFFICIENT_AMOUNT ==
                              mtmShipOrder(mtm,order3)); //negative amount
    matamikyaDestroy(mtm);
    return true;
}
...
static unsigned int makeBadOrder(Matamikya mtm) {
    makeInventory(mtm);
    unsigned int id = mtmCreateNewOrder(mtm);
    mtmChangeProductAmountInOrder(mtm, id, 6, 10.25);
    mtmChangeProductAmountInOrder(mtm, id, 10, 2);
